...
class CartVC: UIViewController {
    ...
    //var cart: [CartModel] = []
    //var wishlistproducts1: [CartModelBody] = []
    ...
    @IBOutlet var startBtnView: UIButton!
        ...
        Networking.instance.getApiCall(url: url){(response, error, statusCode) in
            ...
            if error == nil{
                if let body = response["body"].dictionary {
                    //print(response)
                    if let total = body["total"]?.string{
                        self.totalPrice.text = total
                    }
                    if let subTotal = body["sub_total"]?.string{
    ...
    @objc func plusBtnTapped(sender: UIButton){
        ...
        //if let index = cartProductsArr.firstIndex(where: {$0.quantity == self.quantity}){
            qty = qty + 1
...
extension CartVC : UITableViewDelegate,UITableViewDataSource{
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        //cell.quantityLb.text = isQuantityLbl ? "\(self.quantity)" : "\(product.quantity ?? 0)"
        //let qty = max(1, product.quantity ?? 1)
...
//        let selectedProduct = cartProductsArr[indexPath.row]
//        selectedResponseID = selectedProduct.id
//
//    }
}
