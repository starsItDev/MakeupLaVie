...
class cartTableViewCell: UITableViewCell ,UITableViewDelegate {
    ...
    @IBAction func deleteButtonPressed(_ sender: Any) {
        if let tableView = superview as? UITableView, let indexPath = tableView.indexPath(for: self) {
            if let cartVC = tableView.delegate as? CartVC {
                //cartVC.deleteButtonPressed(id: (sender as AnyObject).tag, at: indexPath)
            }
        }
    }
    
    func changeTotal(instance: CartModelResponse){
