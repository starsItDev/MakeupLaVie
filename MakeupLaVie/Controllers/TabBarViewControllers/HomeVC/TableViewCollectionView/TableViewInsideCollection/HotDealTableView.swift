// HotDealTableView.swift

import UIKit

protocol HotDealCollectionViewDelegate: AnyObject {
    func Wishlist(selectedID: Int?)
    func getCellIndex(_ cell: HotDealCollectionView) -> Int?
}

class HotDealTableView: UITableViewCell, UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout, HotDealCollectionViewDelegate {
    ...
    override func awakeFromNib() {
        super.awakeFromNib()
        configuration()
        self.collectionView.register(UINib(nibName: "HotDealCollectionView", bundle: nil), forCellWithReuseIdentifier: "HotDealCollectionView")
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "HotDealCollectionView", for: indexPath) as! HotDealCollectionView
        
        cell.delegate = self
        
        var currentIndex = 0
        for product in viewModel.products {
            let responseCount = product.body.hotDeals.count
            if indexPath.item >= currentIndex && indexPath.item < currentIndex + responseCount {
                let itemIndex = indexPath.item - currentIndex
                let specialOffer = product.body.hotDeals[itemIndex]
                cell.titleLbl.text = product.body.hotDeals[itemIndex].title
                cell.rsLbl.text = product.body.hotDeals[itemIndex].price
                cell.disLbl.text = product.body.hotDeals[itemIndex].discount
                cell.imgView.setImage(with: product.body.hotDeals[itemIndex].image)
                let wishList = product.body.hotDeals[itemIndex].hasWishlist
...
// MARK: - Configuration and ViewModel
extension HotDealTableView {
    func configuration() {
        initViewModel()
        observeEvent()
    }
